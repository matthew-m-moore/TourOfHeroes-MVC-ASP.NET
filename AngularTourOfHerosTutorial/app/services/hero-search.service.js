...
var HeroSearchService = (function () {
    function HeroSearchService(Http) {
        this.Http = Http;
        this.WebApiUrl = 'api/heroes/search';
        this.Headers = new http_2.Headers({ 'Content-Type': 'application/json' });
    }
    HeroSearchService.prototype.SearchForHero = function (searchTerm) {
        var url = this.WebApiUrl + "?searchTerm=" + searchTerm;
        return this.Http.get(url)
            .toPromise()
            .then(function (response) { return response.json(); })
            .catch(this.handleError);
    };
    HeroSearchService.prototype.SearchForHeroAsync = function (searchTerm) {
        var url = this.WebApiUrl + "?searchTerm=" + searchTerm;
        return this.Http.get(url)
